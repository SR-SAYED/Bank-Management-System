...
void view_list()
{
    ...
    while(fscanf(view,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d",&add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
       {
           printf("\n%6d\t\t%10s\t\t%6s\t\t\t%.0lf",add.acc_no,add.name,add.address,add.phone);
           test++;
       }
...
void close(void)
{
    printf("\nThis Project is made By MD Sayedur Rahman");
}
...
{   int choice;
    ...
    printf("\n\t1.Create new account\n\t2.Update an account information\n\t3.For transactions\n\t4.Check an account details\n\t5.Removing  an account\n\t6.View customer's list\n\t7.Exit\n\tEnter your choice:");
    scanf("%d",&choice);
...
int main()
{
    char pass[10],password[10]="bubt1234";
    int i=0;
